  ...
  return (
    <>
      <PageMasthead
        ...
        standfirst="Paired vice presidents and paired project directors — one for each town. It is the structural rule that keeps the club whole."
      />
            ...
            <li key={member.id} className={index % 2 === 1 ? 'drop-sm' : undefined}>
              <Reveal delay={index * 60}>
                {member.photo ? (
                  ...
                  <div
                    aria-hidden
                    className="flex aspect-[3/4] items-center justify-center bg-accent font-heading text-4xl font-bold text-page"
                  >
                    {getInitials(member.name)}
                  </div>
                )}
                <p className="mt-3 font-heading text-base leading-tight font-bold">{member.name}</p>
