...
/**
 * Header. The wordmark is set as two stacked town names divided by a rule,
 * which is the club's identity stated in three words before you read anything
 * else.
 *
...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-rule bg-page/96 backdrop-blur-sm">
      <div className="wrap flex h-20 items-center justify-between gap-6">
        <Link href="/" className="flex items-center gap-3" aria-label={`${club.name} — home`}>
          <span className="font-heading text-xs leading-[1.1] font-bold tracking-tight uppercase">
            <span className="block text-accent">Panadura</span>
            <span className="my-0.5 block h-px w-6 bg-ink-faint" aria-hidden />
            <span className="block text-warm">Alubomulla</span>
          </span>
          <span className="hidden font-heading text-sm font-bold text-ink sm:block">Leo Club</span>
        </Link>
        ...
        <Link
          href="/join"
          className="hidden shrink-0 bg-accent px-5 py-2.5 text-sm font-bold text-page transition-colors hover:bg-warm lg:inline-block"
        >
          Join
        </Link>
              ...
              return (
                <li key={item.href} className="border-b border-rule last:border-b-0">
                  <Link
                    ...
                    aria-current={active ? 'page' : undefined}
                    className={cn(
                      'block py-4 font-heading text-lg font-bold',
